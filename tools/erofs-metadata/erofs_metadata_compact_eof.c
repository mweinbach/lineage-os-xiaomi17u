...
/* The compact writer appends a PLAIN marker at an unaligned EOF. Its implicit
 * block address can be exactly one past the filesystem when the preceding
 * extent uses the final block. The pinned mapper discards this marker when
 * the queried byte precedes clusterofs, then looks back to the real head.
 * Permit only that unused compact address; a selected head must remain in
 * range. Passing the original query offset also excludes lookback heads. */
static bool discarded_compact_eof_head(const struct erofs_inode *inode,
                                      uint64_t lcn, uint64_t offset,
                                      unsigned type, unsigned clusterofs,
                                      uint64_t pblk)
{
    return inode->datalayout == EROFS_INODE_COMPRESSED_COMPACT &&
           pblk == image_sbi.primarydevice_blocks &&
           type == Z_EROFS_LCLUSTER_TYPE_PLAIN && lcn > 0 &&
           lcn == inode->i_size / 4096U &&
           clusterofs == inode->i_size % 4096U &&
           offset < inode->i_size && offset / 4096U == lcn &&
           offset % 4096U < clusterofs;
}
...
static int raw_lcluster(const struct erofs_inode *inode, uint64_t lcn, uint64_t offset,
        ...
        if (*type == Z_EROFS_LCLUSTER_TYPE_NONHEAD) {
            ...
            if (physical_base > UINT32_MAX - nblk ||
                ((uint64_t)physical_base + nblk >= image_sbi.primarydevice_blocks &&
                 !discarded_compact_eof_head(inode, lcn, offset, *type, *clusterofs,
                                            (uint64_t)physical_base + nblk)))
                return -EINVAL;
        }
    }
...
static int check_lookback(const struct erofs_inode *inode, uint64_t offset)
{
    ...
    int result = raw_lcluster(inode, lcn, offset, &type, &clusterofs, &delta);
    if (result)
        return result;
    if (type != Z_EROFS_LCLUSTER_TYPE_NONHEAD) {
    ...
    for (unsigned step = 0; step < MAX_LOOKBACK_STEPS; ++step) {
        if (!delta || delta > lcn || initial - (lcn - delta) > MAX_LOOKBACK_STEPS)
            return -EINVAL;
        lcn -= delta;
        result = raw_lcluster(inode, lcn, offset, &type, &clusterofs, &delta);
        if (result)
            return result;
        if (type != Z_EROFS_LCLUSTER_TYPE_NONHEAD)
            return 0;
