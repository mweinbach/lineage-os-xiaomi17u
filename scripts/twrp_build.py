...
def controls(config, root=ROOT):
    return {"target_files": target_inventory(root), **patch_inventory(config, root)}
...
def plan(config, paths, host_mode, jobs, variant, root=ROOT):
    ...
    return {**twrp_workspace.identity(config, paths), "action": "plan", "executes_commands": False,
...
def run_build(config, paths, action, host_mode, jobs, variant, root=ROOT):
    ...
    env = environment(paths["source_dir"], paths["out_dir"], variant)
    verified = check(config, paths, host_mode, root)
    ...
    try:
        ...
        with log_path.open("x") as log:
            ...
            check(config, paths, host_mode, root, host=verified["host_preflight"], record=False)
            if action == "build":
...
def main(argv=None):
    ...
    parser.add_argument("action", nargs="?", choices=("plan", "prepare", "check", "graph", "build"), default="plan")
