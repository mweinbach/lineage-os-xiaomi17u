...
include_apex adds only an empty /apex and uses 1536 bytes. Explicit include_config
also adds only empty /config at that size. Each is encoded as a single
literal-only legacy LZ4 block. No decompressor or kernel is executed.
"""
...
CONFIG_DIRECTORIES = ("apex", "config") + DIRECTORIES
CPIO_SIZE = 1024
PREFIX_SIZE = 1037
APEX_CPIO_SIZE = 1536
APEX_PREFIX_SIZE = 1551
CONFIG_CPIO_SIZE = 1536
CONFIG_PREFIX_SIZE = 1551
...
CONFIG_CPIO_SHA256 = "21053c223bbd43731b7ba898bbe75eb5816e9b254a8a53913e145a78a6f6a5e4"
CONFIG_PREFIX_SHA256 = "602e50e43fc8b77e5550d7e01c4461339d5b02541065407a013a39d054324d9f"
...
def _variant(include_apex, include_config=False):
    if type(include_apex) is not bool:
        raise ScaffoldError("include_apex must be an explicit boolean")
    if type(include_config) is not bool:
        raise ScaffoldError("include_config must be an explicit boolean")
    if include_config:
        if not include_apex:
            raise ScaffoldError("include_config requires explicit include_apex")
        return CONFIG_DIRECTORIES, CONFIG_CPIO_SIZE, CONFIG_PREFIX_SIZE, CONFIG_CPIO_SHA256, CONFIG_PREFIX_SHA256
    if include_apex:
...
def build_scaffold_cpio(*, include_apex=False, include_config=False):
    """Return the selected fixed root-owned 0755 directory set and newc trailer."""
    names, archive_size, _, archive_sha, _ = _variant(include_apex, include_config)
...
def build_scaffold_prefix(*, include_apex=False, include_config=False):
    """Return the selected fixed legacy-LZ4 prefix without consulting files/tools."""
    _, archive_size, prefix_size, _, prefix_sha = _variant(include_apex, include_config)
    ...
    block = b"\xf0" + extension + build_scaffold_cpio(include_apex=include_apex, include_config=include_config)
...
def inspect_scaffold_prefix(prefix, *, include_apex=False, include_config=False):
    """Require byte-for-byte identity, including directory metadata and padding."""
    names, archive_size, prefix_size, archive_sha, prefix_sha = _variant(include_apex, include_config)
    if type(prefix) is not bytes or prefix != build_scaffold_prefix(include_apex=include_apex, include_config=include_config):
    ...
    return {
        ...
        "include_apex": include_apex, "include_config": include_config,
        ...
        "empty_config_directory_verified": include_config, "configfs_mount_verified": False, "usb_runtime_verified": False,
