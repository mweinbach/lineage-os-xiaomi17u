...
def _contracts():
    ...
    return _identity(profile_bytes), source, _identity(source_bytes)
...
def _source_check(source_tree, source, contract_identity):
    ...
    return {"expected_project_commit": BUILD_COMMIT, "contract": contract_identity,
...
def plan():
    profile, source, contract = _contracts()
    return copy.deepcopy({"schema_version": BUNDLE_SCHEMA_VERSION, "operation": "plan-nezha-recovery-inputs", "profile_id": PROFILE_ID,
