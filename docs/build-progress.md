...
vendor image. Adding the exact two-permission helper projection produces a
[combined policy prototype](helper-policy-projection.md) that compiles with
zero permissive domains and every assertion retained. This is not installed
in an image or generated by a new Android source build. The product
...
directories outside Repo. The current installed device admission is still v9.

The next local milestones before a complete device build are:

- Bind and admit the helper source capability, validate its actual Android M4
  path, then repeat the full strict composition using newly built source CIL.
- Integrate the reviewed Binder correction into a reproducible derived vendor
  input, preserving the original images, filesystem metadata, provenance and
  AVB requirements. The private CIL experiment has not done that packaging.
