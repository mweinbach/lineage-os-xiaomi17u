...
| [GMS source integration](gms-source-integration.md) | SignApk stage passes but installation fails with verified rollback; active 541-file source and CrossDevice failure remain, with native APK diagnostics distinguished from host parser replay |
| [SignApk source-stamp correction](signapk-source-stamp.md) | Prepared two-file fix removes obsolete stamp metadata before APK signing and restores the normal signer dependency; the later rejected adoption is recorded above, and a successful adoption and strict retry remain pending |
