...
| [GMS source integration](gms-source-integration.md) | Corrected SignApk source adoption passes with 543 files/fourteen projects; successor identity is host-calculated, while native queries, rebuilt signer and strict component retry remain pending; the failed first attempt is preserved |
| [SignApk source-stamp correction](signapk-source-stamp.md) | Frozen two-file patch removes obsolete stamp metadata before APK signing and restores the normal signer dependency; its preparation record is preserved, with later successful adoption and remaining native checks recorded above |
