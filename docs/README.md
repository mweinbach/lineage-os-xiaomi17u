...
| [Optional Qualcomm AIDL namespace exports](../config/nezha-qti-aidl-namespaces.json) | Native input guard verifies 666 files, one explicit alias and thirteen project HEADs; namespace activation, compiled metadata/API/backend checks and definition coverage remain pending |
...
| [Policy3 delivery source selection](../config/nezha-policy-image-delivery-policy3.json) | Committed successor with reviewed private copies and inactive source stage; three-tree adoption fails unchanged disk preflight before candidate creation, leaving source539 active |
