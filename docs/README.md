...
| [AIDL metadata name audit](../tools/vintf-definition-audit/README.md) | Installed audit source; existing metadata lacks 134 matrix names, with native audit compilation/execution and complete compatibility still unverified |
| [Optional Qualcomm AIDL namespace exports](../config/nezha-qti-aidl-namespaces.json) | The current source and nothing3 select the two guarded namespaces; read-only capture v2 completes after the retained v1 bound failure, while producer admission, compilation and definition coverage remain pending |
...
| [Policy3 delivery source selection](../config/nezha-policy-image-delivery-policy3.json) | Reviewed adoption verifies 537 source files; images1 retains native 0/wrapper 1, while a separate artifact postcheck and receipt readback pass with independent review pending; packaging remains unverified |
...
| [Policy-image input preparation](policy-image-inputs.md) | Repeated policy3 raw/NONE-footer outputs are verified and selected as source inputs; ordinary output images, signed parent chain, physical fit and packaging remain pending |
