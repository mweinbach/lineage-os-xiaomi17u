...
| [Optional Qualcomm AIDL namespace exports](../config/nezha-qti-aidl-namespaces.json) | The current source and nothing3 select the two guarded namespaces; first component capture fails its bounded query, with compilation, metadata/API/backend checks and definition coverage still pending |
...
| [Policy3 delivery source selection](../config/nezha-policy-image-delivery-policy3.json) | Reviewed three-tree adoption verifies 537 source files; product queries and nothing3 metadata values pass, while ordinary image and packaging results remain pending |
