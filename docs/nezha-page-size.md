...
The optional `nezha-stock-4k-bringup-v1` profile matches the admitted stock
kernel's actual 4 KiB runtime pages. It is a first-boot configuration choice,
not a claim of Android 16 KiB or VSR compatibility. Normal Android SELinux,
AVB, ELF symbol/dependency checks and the prebuilt alignment check remain
enabled. Complete target-files and flash admission remain false.
...
## Explicit generation and admission

Add this argument to the existing provider-enabled factory generation command:
...
profile. The first native check must confirm maximum 4096, alignment checking
true and no-Bionic-page-size-macro true while retaining other strict checks.
